                     ...
                     OperationEnvironment& env)
{
  ...
  if (declaration.Size() < 2 || declaration.Size() > 10)
    return false;
  ...
  bool success = SendDeclaration(declaration, home, env);

  env.SetProgressPosition(1);

  if (success) {
    for (uint8_t tp = 1; tp <= declaration.Size(); tp++) {
      success = SendObsZone(tp, declaration, env);
      if (!success)
        break;
  ...
  if (success)
    success = SendCompetitionClass(declaration, env);
                             ...
                             OperationEnvironment& env)
{
  ...
  CopyStringSpacePadded(
    data.pilot, declaration.pilot_name, sizeof(data.pilot));
  CopyStringSpacePadded(
    data.glider, declaration.aircraft_type, sizeof(data.glider));
  CopyStringSpacePadded(
    data.reg_num, declaration.aircraft_registration, sizeof(data.reg_num));
  CopyStringSpacePadded(
    data.cmp_num, declaration.competition_id, sizeof(data.cmp_num));
  ...
  data.crc = CalculateCRC(data_array + 2, sizeof(data) - 3, 0xFF);
  std::span<std::byte> message(data_array, sizeof(data));

  return WriteAndWaitForACK(message, env);
}

bool
LXEosDevice::SendObsZone(uint8_t tp_nr,
